...
function Glaze(id) {
	...
	thisObj.lastVariation = function() {
		// Returns the last variation object in the glaze
		return thisObj.variations[ thisObj.variations.length-1 ];
	}
	
	thisObj.sumOfBase = function() {
		// Returns the sum of the base ingredients
		var sumOfBase = 0;
		for (var i in thisObj.recipe) {
			sumOfBase += parseFloat(thisObj.recipe[i].IngredientAmt);
		}
	...
	thisObj.calcPW = function(batch) {
		// Recalculates ingredient amounts
		// to a given batch size
		
		batch = (!batch)? 100: batch;			// set defualt batch size to 100
		
		// calculate sum of base recipe
		baseSum = thisObj.sumOfBase();

		// divide each base ingredient by sum of base
		for (var i in thisObj.recipe) {
			thisObj.recipe[i].IngredientAmt /= baseSum;
			thisObj.recipe[i].IngredientAmt *= batch;
		...
		for (var i in thisObj.variations) {
			for (var j in thisObj.variations[i].recipe) {
				...
				thisObj.variations[i].recipe[j].IngredientAmt *= batch;
				...
				if (glazeRes['glazeObj']['GlazeID'] == 0) {
					...
					for (var i in glazeRes['varList']) {
						...
						thisObj.defaultVar = thisObj.variations[0];		// Default variation is set using ORDER BY in sql
																		// Again, this is bad coding, because what if defaultVar is changed, or assigned somewhere else
																		// Should use a glaze.getDefaultVar() method instead.
					}
	...
	thisObj.saveGlaze = function() {
		...
		*/
		...
		$.post(ajaxFile, {'data':data}, function(res) {
			// Update glaze object
			var resGlaze = $.evalJSON(res);					// Returns the glaze object, with any new auto-IDs
			
			// Set any new auto-IDs
			thisObj.GlazeID = resGlaze.GlazeID;
			thisObj.id = thisObj.GlazeID;
			// Update any new variation IDs
			for (var i in resGlaze.variations) {
				thisObj.variations[i].GlazeID = resGlaze.variations[i].GlazeID;
				thisObj.variations[i].VarID = resGlaze.variations[i].VarID;
				thisObj.variations[i].id = resGlaze.variations[i].VarID;
				// Update any new Image IDs
				for (var j in resGlaze.variations[i].images) {
					thisObj.variations[i].images[j].ImageID = resGlaze.variations[i].images[j].ImageID;
					thisObj.variations[i].images[j].id = resGlaze.variations[i].images[j].ImageID;
				}
			}
	...
	thisObj.getUnityTable = function () {
		...
		for(var i in glaze.unity) {
			...
			// Format subscript numbers in formula
			var oxideFormula = strToSub(glaze.unity[i].OxideFormula);
			
			var cell_formula = $('<td></td>').html(oxideFormula);									// Create formula cell
