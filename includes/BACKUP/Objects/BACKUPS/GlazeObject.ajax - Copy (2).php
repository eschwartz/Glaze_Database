...
function saveGlaze($glazeObj) {
	...
	*/
	...
	$glazeIngredientsSQLFields = getSQLFields('GlazeIngredients');
	...
	/* Update `Glazes` table*/
		...
		foreach ($glazeObj as $prop => $val) {
		 	if (in_array($prop,$glazesSQLFields) ) {			// Only updates from Glaze Object properties that are also fields in `Glazes` table
