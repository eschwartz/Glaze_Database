...
// calculateUnity		$recipe	(ingrID => amt, base)		$unityOxide (oxideID => oxideAmt)		MASTER FUNCTION.
...
// And send back 
if(is_ajax()) {	
	$recipe = json_decode(stripslashes($_POST['data']), true); 	// strpslashes requires to remove \" (magic quotes is on)
	$unityOxide = calculateUnity($recipe);
	$unityHTMLTable = getUnityTable($unityOxide);
	// echo json_encode($unityOxide); 	// Use to return an array of $unityOxide (oxideID => unity amt)
	echo $unityHTMLTable;				// Use to return an HTML table of the unity formula
}
elseif ($_REQUEST['glazeID']) {
	$glazeID = $_REQUEST['glazeID'];
...
elseif ($_GET['test'] == 'yes') {
	...
	$testRecipe[3] = 12;
	$testRecipe[4] = 5;
	echo "<h3> Test Recipe</h3>";
	$unityOxide = calculateUnity($testRecipe);
	echo "<h4>Return is: </h4>";
	printArray($unityOxide);
	echo "<h4>Unity Table</h4>";
	echo getUnityTable($unityOxide);
	echo "<h4>Oxide Formulas (by oxideID)</h4>";
	printArray(createIDValue('OxideID', 'OxideFormula', 'Oxides'));
}
