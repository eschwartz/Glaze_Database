...
// createOxideArray() returns $oxideArry('formula') = oxideID
...
if ($_POST['submit']) {
	if ($_FILES["importFile"]["error"] > 0)
	  ...
	  move_uploaded_file($_FILES["importFile"]["tmp_name"],
      "files/" . $_FILES["importFile"]["name"]);
...
function createTempTable($fileLocation) {
	...
	$sql = 	  "CREATE TABLE temp_ingrImport\n"
			. "("
			. "IngredientImportID int(5), IngredientName varchar(55), IngredientLOI decimal(10,5),	IngredientTE decimal(10,6),\n"
...
function transferTempData() {
	...
	$sql = "SELECT IngredientImportID, IngredientName, IngredientTE, IngredientLOI\n"
			. "FROM temp_ingrImport";
	$rs = mysql_query($sql, $conn) or die("Could not execute query: " . mysql_error() . "<br>SQL is: $sql");
	...
	$sql_insertIngr = "";
	while ($row = mysql_fetch_array($rs)) {
		$sql_insertIngr .= "($row[IngredientImportID],\"$row[IngredientName]\",$row[IngredientTE],$row[IngredientLOI]),";
	}
	$sql_insertIngr = substr($sql_insertIngr,0,-1);		// Removes last "," from string

	$sql = "INSERT into Ingredients (IngredientImportID, IngredientName, IngredientTE, IngredientLOI)\n"
...
<title>DYLMG - import ingrs</title>
...
<h1>Import Ingredients</h1>
<h3>Need security on this page before going live!</h3>
