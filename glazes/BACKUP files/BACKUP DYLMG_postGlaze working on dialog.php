...
$(document).ready(function() {
	...
	// Set properties for newVar dialog
	// http://jqueryui.com/demos/dialog/#modal-form
	// See link for validation code
	var glazeDialogBtns = new Object();

	$(function () {
		// Sets variables for each input field
		var newVar_name = $(this).find('#newVar_name'),
			newVar_descr = $(this).find('#newVar_descr'),
			newVar_color = $(this).find('#newVar_color'),
			newVar_surface = $(this).find('#newVar_surface'),
			allFields = $( [] ).add(newVar_name).add(newVar_descr).add(newVar_color).add(newVar_surface);
		
		// Define buttons
		glazeDialogBtns['Add New Glaze'] = function() {
						...
						varInfo = {'VarID': 'new', 
									...
									'VarDefault': 0								// If it is new, it is not default (original)
									};			
		...
		$('<div></div>').dialog({
			...
			closeOnEscape: false,
			close: function() { alert($(this).html()); }
		...
		// Open dialog with "Add New Variation" button
		$('#input_newVar').button().click(function() {
			// auto save before creating new var
			saveGlaze();
			
			// Use new buttons array (renames 'Add New Glaze' , and adds 'Cancel')
			varBtns = new Object();
			varBtns['Add new glaze variation'] = glazeDialogBtns['Add New Glaze'];
			varBtns['Cancel'] = function() {
				$(this).dialog('close');
			};
			$('div.dialog_newInfo').dialog('option', 'buttons', varBtns);
			
			// Change labels from "Variation" -> "Glaze"
			/*$('#dialog_newVar label').each(function() {
				var labelText = $(this).html();
				var glazeText = labeltext.replace("Variation", "Glaze");
				$(this).html(glazeText);
			});*/
			
			
			$('#dialog_newVar').dialog('open');		// opens dialog (previously defined above)							 
		});
	});	// end of infoDialog()
...
window.onload = function () {
	...
	if (glazeInfo['GlazeID'] != 'new') {
		loadVariation('list');					// sets varInfo (varID => FieldName => value)
...
// Or loads list of variations if varID = 'list'
	...
	$.post(ajaxFile, {'varID': varID, 'glazeID': glazeInfo['GlazeID'], 'glazeAuthor': glazeInfo['GlazeAuthor']}, function(res) {
		var resArray = $.evalJSON(res);
		
		// Build list of variations
		if(varID == 'list') {
			var varInfo = resArray['varInfo'];
			if (varInfo != 'none') {	// If variations exist for this glaze
				buildVarsList(varInfo);
			}
		}
...
function buildVarsList(varInfo) {
	$('#table_vars tbody').empty();
	for (var variation in varInfo) {
		for (var fieldName in varInfo[variation]) {
			var varName = varInfo[variation]['VarName'];
			var varColor = varInfo[variation]['VarColor']; 
			var varID = varInfo[variation]['VarID'];
