...
$(document).ready(function() {
	...
	// Gets list of Vars, and new or default var recipe
	var varInfo = new Object();
	var varRecipe = new Object();
...
function loadVariation(varID) {
	...
	$.post(ajaxFile, {'varID': varID, 'glazeID': glazeInfo['GlazeID'], 'glazeAuthor': glazeInfo['GlazeAuthor']}, function(res) {
		$('#alert').html("<h1>loadVariation complete</h1>");
	...
	$.post(ajaxFile, "filter="+ingrFilter, function(res) {
		...
		if (ingrIDName.length > 0) {
			...
			for (var i=0; i<ingrIDName.length; i++) {
				...
				$('#tbl_suggest').append("<tr class='ingrRow' id='"+ingrID+ "'><td><div id='ingrBase' style='visibility:hidden; width:0px; height:0px'>"+ingrBase+"</div><div id='ingrName'>"+ingrName+"</div></td></tr>");
...
// Clears and rebuilds recipe table from recipe array
	...
	var cell_ingrAmt = "<td><input type='text' name='" + ingrID + "' value='" + ingrAmt + "' class='ingrAmt' size='5' /></td>";
	...
	// Change Recipe Ingr Amt => update recipe, and reload unity
