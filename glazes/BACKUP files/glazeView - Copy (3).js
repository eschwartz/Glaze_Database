...
// These run AJAX with ({async: false})
...
function displayVariation(varObj) {
	...
	// Load images to Image object
	asyncLoading(varObj.loadVarImages, function() {			// Waits for Image object to load from AJAX before continuing
		// Load images to page
		$('ul#thumbGallery').empty();
		$('div#loadedImg').empty()
		if (varObj.images.length > 0) {
			$('div#loadedImg').append( $('<img>').attr('src', varObj.images[0].ImageSrc) );			// set first image as loaded image
				
			// Load hoverGallery
			for(var i in varObj.images) {
				var imgObj = varObj.images[i];
				$('ul#thumbGallery').append(imgObj.getHoverLi() );
			}
			hoverGallery();
		} else {
			$('div#loadedImg').append('<img src=/images/no_image.gif>');
		}
	}, $('div#loadedImg'), 50, 10000);		// Check dirty in last image ajax call
