...
var glaze = new Glaze(GET_glazeID);		// Set glazeID to 1 for TESTING ONLY
		...
		$('#dialog_newGlaze').dialog( {
			...
			buttons: {
				"Create Glaze": function() {
					...
					var defaultVar = new Variation();
					$(this).find('input,textarea,select').each(function() {
						defaultVar[$(this).attr('id')] = $(this).val();
					});
					
					/** WHAT TO DO HERE NEXT **/
					/**
					 * run a glaze.createGlaze() function, that saves the glaze and gets a new glazeID
					 * run a glaze.addVariation(defaultVar) function, that adds the variation object to the glaze object (a simple push() will do)
					 * Save the glaze, so default variation info is saved.
					*/
				},
				"Cancel": function() {
