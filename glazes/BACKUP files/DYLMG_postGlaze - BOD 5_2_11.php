...
// Get arrays from php and declare global variables
...
// Should really define glazeName here as a global variable, by calling loadVariation(default).... 
// Actually, all default glazeInfo should be saved. For example in details, want to always show default glaze author.
...
// Or loads list of variations if varID = 'list'
	...
	$.post(ajaxFile, dataJSON , function(res) {
		...
		// Load variation recipe into Additional Ingr table
			...
			var  datePosted = varInfo['VarDatePosted'];
			...
			//if (isVarAuthor == true) {			// All of this author verification will be handled... eventually.
			...
			// Set Glaze Details
			$('div#details_name').html(varInfo['VarName']);
			$('div#details_author').html("<a href='/usr/userProfile.php?userID="+varInfo['VarAuthor']+"' style='font-weight:normal' >"+varInfo['UserLogin']+"</a>");
			$('div#details_descr').html(varInfo['VarDescr']);
			$('div#details_color').html(varInfo['VarColor']);
			$('div#details_surface').html(varInfo['VarSurface']);
			$('div#details_datePosted').html(datePosted);
		}
...
// Loads list variations into table
	...
	for (var variation in varList) {
		for (var fieldName in varList[variation]) {
			...
			var highlightClass = ""; 
			if (varList[variation]['VarDefault'] == 1) {
				$('#glazeTitle').html(varList[variation]['VarName']);
				highlightClass = 'ui-state-highlight';					// Sets highlight to default loaded glaze 
			}
		...
		$('#table_vars tbody').append("<tr class="+highlightClass+">"+cell_varName+cell_varColor+"</tr>");
	}
	
	// EVENT: Click->Loads selected variation
	$('a.selectVar').click(function() {
		var varID = $(this).siblings('div.varID').html();		// Sets varID to new loaded variation
		loadVariation(varID);									// Variation recipe
		$('#table_vars').find('tr').removeClass('ui-state-highlight');	// Removes highlight from all rows
		$(this).parents('tr').addClass('ui-state-highlight');			// Highlights selected Variation row
	});
...
// Creates table of glaze-image thumbnails
	...
	if (varImages.length < 1) {
		...
		for (var i=0; i<varImages.length; i++ ) {
			...
			var imageParams = new Object; 			// To send to buildHoverDescr in jsFunctions -> make alt attr. for hover <img>
			imageParams['VarName'] = $('#glazeTitle').html() +" + "+ varInfo['VarName'];		// bad way to get glaze title, should be declared global var
			imageParams['VarDescr'] = varImages[i][1];
			imageParams['VarColor'] = "  ";				// Could use these places for 
			imageParams['VarSurface'] = "  ";			// some kind of subhead...
			imageParams['UserID'] = varImages[i][2];
			imageParams['UserLogin'] = varImages[i][3];
			
			var imageDescr = buildHoverDescr(imageParams);		// Sets us formatting for image hover info
...
// Creates and/or Filters ingr autosuggest options by search text
	...
	if ($('#tbl_suggest').length == 0) {
		...
		// Set width of suggest table relative to column
		var columnWidth = $('#table_recipe').find('td').first().css('width');
		$('#tbl_suggest').width(columnWidth);
		
		/*var inputWidth = parseFloat(el_ingrFilter.css('width'));		// Sets width to size of input
		$('#tbl_suggest').width(inputWidth*2); */
...
<div id='content-container' >
	...
	<div id='content-column'>
		...
		<div class='section' style="margin-top:2px;">
			...
			<button onclick="calcPW()" name="btn_calcPW" id="btn_calcPW" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false"><span class="ui-button-text">Calculate PW</span></button>	<!--NEED onclick='calcPW()' in JS for this button -->
	
			<!-- Save Glaze-->
			<button onclick="saveGlaze()" name="saveForm"  class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button" aria-disabled="false"><span class="ui-button-text">Save Glaze</span></button>	<!-- NEED onclick="saveGlaze()" in javascript for button -->
